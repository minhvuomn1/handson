document.addEventListener("DOMContentLoaded", function () {
    ...
    const locations = [
        ...
        'Cà Mau', 'Cần Thơ', 'Cao Bằng', 'Đắk Lắk', 'Đắk Nông', 'Điện Biên', 'Đồng Nai', 'Đồng Tháp', 'Gia Lai', 'Hà Giang', 'Hà Nam', 'Hà Nội', 'Hải Dương', 
        'Hải Phòng', 'Hậu Giang', 'Hòa Bình', 'Hồ Chí Minh', 'Hưng Yên', 'Khánh Hòa', 'Kiên Giang', 'Kon Tum', 'Lai Châu', 'Lâm Đồng', 'Lạng Sơn', 'Lào Cai', 
        'Long An', 'Nam Định', 'Nghệ An', 'Ninh Bình', 'Ninh Thuận', 'Phú Thọ', 'Phú Yên', 'Quảng Bình', 'Quảng Nam', 'Quảng Ngãi', 'Quảng Ninh', 'Quảng Trị', 
        'Sóc Trăng', 'Sơn La', 'Tây Ninh', 'Thái Bình', 'Thái Nguyên', 'Thanh Hóa', 'Thừa Thiên-Huế', 'Tiền Giang', 'TP.HCM', 'Trà Vinh', 'Tuyên Quang', 
    ...
    function renderScholarshipCards(students) {
        ...
        students.forEach((item) => {
            ...
            clone.querySelector(".student-name").textContent = student.name || "Không rõ";
            clone.querySelector(".student-school").textContent = school.name || "Không rõ";
            clone.querySelector(".grade").textContent = student.grade || "N/A";
            clone.querySelector(".dob").textContent = student.dob || "N/A";
            clone.querySelector(".generation").textContent = student.generation || "Không rõ";
            clone.querySelector(".location").textContent = student.location || "Không rõ";
            clone.querySelector(".sponsors").textContent = sponsors.name || "Không rõ";

            const typeDiv = clone.querySelector(".scholarship-type");
            const typeText = scholarship.type || "Không rõ";
            typeDiv.textContent = typeText;
            typeDiv.classList.add(typeText.includes("Học bổng toàn phần") ? "full" : "partial");
    ...
    filterBtn?.addEventListener("click", () => {
        ...
        const filtered = allStudents.filter(item => {
            ...
            const studentYear = studentDob ? new Date(studentDob).getFullYear() : "";
